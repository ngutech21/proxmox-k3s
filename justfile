...
bootstrap-cert-manager:
  ...
  kubectl -n cert-manager rollout status deploy/cert-manager-cainjector --timeout=180s
  kubectl -n cert-manager rollout status deploy/cert-manager-webhook --timeout=180s
  kubectl apply -f manifests/cert-manager/00-selfsigned-clusterissuer.yaml
  kubectl apply -f manifests/cert-manager/01-root-ca-certificate.yaml
  kubectl apply -f manifests/cert-manager/02-ca-clusterissuer.yaml
